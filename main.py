"""
Sniper Bot V45 — ONE-WAY MODE
=========================================================================
FIXES aplicados:
  1. calc_qty: aplica LEVERAGE correctamente
  2. calc_qty: valida notional real antes de forzar min_qty
  3. place_sl_tp: coloca SL/TP como órdenes reales en BingX (no solo Python)
  4. manage_positions: limpia half_closed cuando posición ya no existe
  5. positionSide = "BOTH" (ONE-WAY mode, correcto para cuentas default BingX)
"""
...
MAX_RISK_PCT     = float(os.getenv("MAX_RISK_PCT",   "0.5"))
...
class BingXClient:
    ...
    async def open_long(self, symbol, qty) -> dict:
    ...
    async def open_short(self, symbol, qty) -> dict:
    ...
    async def place_sl_tp(self, symbol: str, direction: str,
                          qty: float, sl: float, tp: float):
        """
        ✅ FIX: coloca SL y TP como órdenes reales en BingX.
        En ONE-WAY mode siempre positionSide=BOTH + reduceOnly=true.
        """
        ...
        try:
            ...
            log.info(f"SL colocado {symbol} @ {sl:.8f}")
        except Exception as e:
            log.warning(f"SL error {symbol}: {e}")
            await tg(f"⚠️ SL no colocado `{symbol}` @ `{sl:.6f}`\n`{e}`")
        ...
        try:
            ...
            log.info(f"TP colocado {symbol} @ {tp:.8f}")
        except Exception as e:
            log.warning(f"TP error {symbol}: {e}")
            await tg(f"⚠️ TP no colocado `{symbol}` @ `{tp:.6f}`\n`{e}`")
    ...
    async def cancel_all_orders(self, symbol) -> dict:
        try:
            return await self._post("/openApi/swap/v2/trade/cancelAllAfter",
                                    {"symbol": symbol, "type": "activate",
                                     "timeOut": "0"})
        except:
            return {}
...
def analyze(ticker, candles, candles_1h, funding) -> Optional[CoinResult]:
    ...
    log.info(f"{sym}: score={score} dir={direction} "
             ...
             f"slp={slp_now:.1f} adx={adx_now:.1f} rvol={rvol_now:.2f}")
...
def calc_qty(balance: float, entry: float, sl: float) -> float:
    dist = abs(entry - sl)
    if dist < 1e-10:
        log.warning("SL demasiado cercano al entry")
        return 0.0

    # ✅ FIX 1: aplicar leverage al cálculo de posición
    risk_usd  = balance * MAX_RISK_PCT / 100
    qty_risk  = (risk_usd * LEVERAGE) / dist

    # ✅ FIX 2: cap por MAX_POS_USDT
    qty_max   = MAX_POS_USDT / max(entry, 1e-10)
    qty       = min(qty_risk, qty_max)

    # Redondeo según precio del activo
    ...
    # ✅ FIX 3: validar notional real ANTES de forzar mínimo
    real_notional = qty * entry
    if real_notional < 5.0:
        log.warning(
            f"Notional ${real_notional:.2f} < $5 mínimo BingX | "
            f"balance={balance:.2f} dist={dist:.8f} entry={entry:.6f}"
        )
        return 0.0

    log.info(f"qty: bal={balance:.2f} risk=${risk_usd:.3f} lev={LEVERAGE}x "
             f"entry={entry:.6f} dist={dist:.8f} notional=${real_notional:.2f} → {qty}")
    return qty
...
async def run_diagnostics():
    ...
    if balance > 0:
        try:
            k     = await exchange.get_klines("BTC-USDT", "15m", 5)
            entry = k[-1]["close"]
            sl_sim = entry - entry * 0.01
            qty_sim = calc_qty(balance, entry, sl_sim)
            lines.append(f"\n📐 Sim BTC entry=`{entry:.0f}` sl=`{sl_sim:.0f}` → qty=`{qty_sim}`")
        except: pass
...
async def open_trade(cr: CoinResult) -> bool:
    ...
    try:
        ...
        if cr.direction == "LONG":
            await exchange.open_long(sym, qty)
        else:
            await exchange.open_short(sym, qty)

        # ✅ FIX: colocar SL/TP como órdenes reales en BingX
        await asyncio.sleep(0.5)   # pequeña espera para que la posición esté activa
        await exchange.place_sl_tp(sym, cr.direction, qty, cr.sl, cr.tp)
        ...
        log.info(f"✅ TRADE ABIERTO {cr.direction} {sym} qty={qty} "
                 f"val={pos_value:.2f}$ sl={cr.sl:.6f} tp={cr.tp:.6f}")
        return True
...
async def manage_positions():
    try:
        ...
        # ✅ FIX: limpiar half_closed para símbolos sin posición activa
        stale = half_closed - active_syms
        for sym in stale:
            half_closed.discard(sym)
            log.info(f"half_closed limpiado: {sym} ya no tiene posición")
        ...
        for pos in positions:
            ...
            log.info(f"POS {sym} {direction} cur={cur:.6f} "
                     f"sl={sl:.6f} tp={tp:.6f} pnl={pnl_pct:+.2f}% unreal={unr:+.2f}")

            # SL manual (backup si la orden real falla)
            if sl > 0 and ((is_long and cur <= sl) or (not is_long and cur >= sl)):
                try:
                    await exchange.close_all(sym)
                    await tg(f"🛑 *SL TOCADO* `{sym}` {direction}\n"
            ...
            # Cierre parcial 50% al llegar a 0.6% PnL

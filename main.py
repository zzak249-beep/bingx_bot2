"""
main.py — SMC Bot BingX v3.0 [MÁXIMO RENDIMIENTO]
✅ Filtro de correlación entre pares (no abre BTC+ETH mismo lado)
✅ Notificaciones con OB, BOS, CHoCH, MTF, Rango Asia
✅ Anti-hedge reforzado
✅ Reporte horario mejorado con score/12
"""
...
log.info("=== ARRANQUE SMC BOT v3.0 ===")
    ...
    for grupo in GRUPOS_CORRELACION:
        ...
        for par_abierto, pos in posiciones.items():
            if par_abierto in grupo and par_abierto != par:
                if pos["lado"] == lado:
                    log.info(f"[CORR] {par} {lado} bloqueado — {par_abierto} ya abierto en mismo lado")
                    return True
    return False
...
def main():
    ...
    log.info(f"  TRADE: ${config.TRADE_USDT_BASE} base + compounding")
    log.info(f"  LEV:{config.LEVERAGE}x | MAX_POS:{config.MAX_POSICIONES} | TF:{config.TIMEFRAME}")
    log.info(f"  SCORE≥{config.SCORE_MIN}/12 | SOLO_LONG:{config.SOLO_LONG} | DEMO:{config.MODO_DEMO}")
    log.info(f"  ACTIVO: MTF={config.MTF_ACTIVO} OB={config.OB_ACTIVO} BOS={config.BOS_ACTIVO}")
    ...
    log.info(f"Total pares: {len(pares)}")

    _notif(
        f"🤖 *{config.VERSION}* arrancado\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"💰 Balance    : `${balance:.2f} USDT`\n"
        f"💵 Trade size : `${memoria.get_trade_amount():.2f} USDT`\n"
        f"📊 Pares      : `{len(pares)}`\n"
        f"🏅 Score≥`{config.SCORE_MIN}/12` | Lev:`{config.LEVERAGE}x` | Max:`{config.MAX_POSICIONES}` pos\n"
        f"🆕 MTF · OB · BOS/CHoCH · Asia · Correlación · Vela conf.\n"
    ...
    while True:
        try:
            ...
            if len(estado.posiciones) < config.MAX_POSICIONES:
                ...
                log.info(f"Escaneando {len(pares_scan)} pares | KZ:{kz['nombre']} | Score≥{config.SCORE_MIN}/12")

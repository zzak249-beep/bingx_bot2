...
==========================================================
CORRECCIONES v4-FIX (bugs que impedian señales):

  BUG CRITICO 1 — cooldown_clear=False justo cuando hay señal
    CAUSA: bars_since se resetea a 0 cuando rf_sig != 0 (hay señal nueva),
           y cooldown_clear = (bars_since >= COOLDOWN_BARS) = (0 >= 5) = False.
           Esto bloquea long_sig/short_sig exactamente cuando deberian activarse.
    FIX:   cooldown_clear se evalua ANTES de resetear bars_since.
           El cooldown es para señales REPETIDAS, no para la señal inicial.
           Lógica correcta: si acabas de cambiar de tendencia, cooldown_clear=True.

  BUG CRITICO 2 — Variables .env con nombres distintos al código
    CAUSA: El .env usa TRAILING_ACTIVATE_PCT y TRAILING_PROTECT_PCT,
           pero el código lee TRAILING_START_PCT y TRAILING_LOCK_PCT.
           Resultado: los valores del .env se ignoran.
    FIX:   Se añaden aliases en clean() para leer ambos nombres.
           TRAILING_START lee TRAILING_ACTIVATE_PCT y TRAILING_START_PCT.
           TRAILING_LOCK lee TRAILING_PROTECT_PCT y TRAILING_LOCK_PCT.

  BUG CRITICO 3 — MAX_LOSS_PCT="8%" — el "%" rompe clean() en edge cases
    CAUSA: clean() con regex puede fallar si el valor tiene sufijos como "%".
    FIX:   Se añade strip del caracter "%" antes del parseo.

  BUG MODERADO 4 — bars_since no persiste correctamente entre ciclos
    CAUSA: En el primer ciclo con señal, state['bars_since'] se actualiza
           a 0 y se guarda, pero en el siguiente ciclo se lee como 1,
           luego 2, etc. — nunca alcanza COOLDOWN_BARS antes de la proxima señal.
    FIX:   La logica de persistencia es correcta pero se clarifica con
           el fix del BUG 1. bars_since cuenta barras desde la ULTIMA señal,
           no desde el inicio. El check cooldown_clear usa el valor PREVIO.

  BUG MODERADO 5 — hmac.new deberia ser hmac.new (Python 3 usa hmac.new OK)
    No es bug pero se mantiene como hmac.new() que es correcto.

CORRECCIONES v4 originales (basadas en liquidaciones reales):
  [ver historial de versiones anterior]
"""
...
def clean(key, default, typ='str', aliases=None):
    """
    Lee variable de entorno con soporte de aliases.
    aliases: lista de nombres alternativos a probar si el key principal no existe.
    """
    val = os.getenv(key)
    ...
    # FIX BUG 3: eliminar sufijos como "%" antes de parsear numeros
    if typ in ('int', 'float'):
        v = v.replace(',', '.').rstrip('%')
        m = re.match(r'^-?\d+\.?\d*', v)
...
# FIX BUG 2: aliases para compatibilidad con nombres alternativos en .env
TRAILING_START   = clean('TRAILING_START_PCT',    '1.5',   'float',
                          aliases=['TRAILING_ACTIVATE_PCT'])
TRAILING_LOCK    = clean('TRAILING_LOCK_PCT',     '50',    'float',
                          aliases=['TRAILING_PROTECT_PCT'])

# FIX BUG 3: strip de "%" incluido en clean()
...
MIN_SCORE     = clean('MIN_SCORE',           '9',     'int')
...
class FloopBot:
    ...
    def __init__(self):
        ...
        log.info(f"  MaxLoss:     -{MAX_LOSS_PCT}% (cierre emergencia)")
        ...
        self._tg(
            f"<b>FLOOP Pro v4-FIX iniciado</b>\n"
    ...
    def _floop_tf(self, symbol, interval, state=None):
        ...
        FIX BUG 1: cooldown_clear se evalua con el valor PREVIO de bars_since,
        no con el valor actualizado. De esta manera, cuando hay señal nueva
        (rf_sig != 0), cooldown_clear refleja cuantas barras han pasado desde
        la señal ANTERIOR, que es lo correcto.
        """
        ...
        # FIX BUG 1: leer bars_since ANTES de actualizarlo
        # bars_since_prev = cuantas barras han pasado desde la ultima señal
        # Esto es lo que determina si estamos en cooldown o no
        bars_since_prev = state.get('bars_since', 999)

        # Ahora actualizamos: si hay señal nueva, resetear a 0; si no, incrementar
        if rf_sig != 0:
            bars_since_new = 0
        else:
            bars_since_new = min(bars_since_prev + 1, 9999)

        state['bars_since'] = bars_since_new

        # FIX BUG 1: cooldown_clear usa bars_since_prev
        # Si hay señal nueva (rf_sig != 0):
        #   - bars_since_prev es cuanto tiempo pasó desde la señal anterior
        #   - Si pasaron >= COOLDOWN_BARS barras, la señal es válida
        #   - Si no pasaron suficientes barras, es señal repetida en poco tiempo
        # Si no hay señal (rf_sig == 0):
        #   - cooldown_clear no importa porque long_sig/short_sig requieren rf_sig != 0
        cooldown_clear = bars_since_prev >= COOLDOWN_BARS
    ...
    def analyze(self, symbol):
        ...
        # Dirección con filtros BTC y mercado
        direction = None
        ...
        return {
            ...
            'bars_since':   main['bars_since_prev'],
            'cooldown_ok':  main['cooldown_clear'],
        }
    ...
    def _cond_order(self, symbol, direction, qty_c, stop_price, otype):
        ...
        try:
            ...
            if is_tp:
                ...
                ok = d.get('code') == 0
                if ok:
                    log.info(f"  TP maker OK @ ${stop_price:.6f}")
                else:
                    ...
                    ok = d2.get('code') == 0
                    if ok:  log.info(f"  TP taker OK fallback @ ${stop_price:.6f}")
                ...
                ok = d.get('code') == 0
                if ok:
                    log.info(f"  SL maker OK trigger=${stop_price:.6f} limit=${limit_price:.6f}")
                else:
                    ...
                    d2 = bingx_request('POST', '/openApi/swap/v2/trade/order', p2).json()
                    ok = d2.get('code') == 0
                    if ok:  log.info(f"  SL taker OK fallback @ ${stop_price:.6f}")
    ...
    def _close_position(self, symbol, direction, t):
        ...
        if qty_c and qty_c > 0 and cur_price > 0:
            ...
            if d.get('code') == 0:
                log.info(f"  Cierre maker IOC @ ${limit_price:.6f}")
                return True
        ...
        if ok: log.info(f"  Cierre taker market OK")
        return ok
    ...
    def open_trade(self, symbol, sig):
        ...
        if not sl_ok:
            log.error(f"  CRITICO: SL no se pudo colocar en {symbol} — cerrando")
            self._tg(
    ...
    def _reporte_horario(self):
        ...
        self._tg(
            f"<b>Reporte horario — FLOOP Pro v4-FIX</b>\n"
    ...
    async def run(self):
        log.info("\n> Bot FLOOP Pro v4-FIX arrancado\n")

"""
analizar.py — Motor de señales SMC v3.0 [MÁXIMO RENDIMIENTO]
✅ Multi-TimeFrame (MTF): confirma señal 5m con tendencia 1h
✅ Order Blocks (OB): última vela bajista/alcista antes de impulso
✅ Break of Structure (BOS) + Change of Character (CHoCH)
✅ Rango Asia (00:00-04:00 UTC) como zona soporte/resistencia
✅ Confirmación de vela (engulfing, pin bar, marubozu)
✅ RSI Wilder correcto (igual que TradingView)
✅ FVG limitado últimas 20 velas
✅ Filtro de volumen en vela actual
...
# ══════════════════════════════════════════════════════════════
# SEÑAL PRINCIPAL v3.0 — Score máximo: 12 puntos
# ══════════════════════════════════════════════════════════════
...
def analizar_par(par: str):
    try:
        ...
        bull_trend_1h = htf in ("BULL", "NEUTRAL")
        bear_trend_1h = htf in ("BEAR", "NEUTRAL")
        ...
        if ob["bull_ob"] and ob["bull_ob_bottom"] <= precio <= ob["bull_ob_top"] * 1.005:
            sl_long  += 2; ml_long.append("OB")
        if ob["bear_ob"] and ob["bear_ob_bottom"] * 0.995 <= precio <= ob["bear_ob_top"]:
            sl_short += 2; ml_short.append("OB")

        # BOS / CHoCH +1
        if bos["bos_bull"]:
            sl_long  += 1; ml_long.append("CHoCH" if bos["choch_bull"] else "BOS")
        if bos["bos_bear"]:
            sl_short += 1; ml_short.append("CHoCH" if bos["choch_bear"] else "BOS")
        ...
        # MTF 1h +1
        if htf == "BULL": sl_long  += 1; ml_long.append("MTF_1H")
        if htf == "BEAR": sl_short += 1; ml_short.append("MTF_1H")

        # EMA 5m +1
        if bull_trend_5m: sl_long  += 1; ml_long.append("EMA")
        if bear_trend_5m: sl_short += 1; ml_short.append("EMA")

        # RSI +1
        if rsi <= config.RSI_BUY_MAX:  sl_long  += 1; ml_long.append(f"RSI{rsi:.0f}")
        if rsi >= config.RSI_SELL_MIN: sl_short += 1; ml_short.append(f"RSI{rsi:.0f}")

        # Confirmación vela +1
        if vela_conf_long:  sl_long  += 1; ml_long.append("VELA")
        if vela_conf_short: sl_short += 1; ml_short.append("VELA")

        # Condiciones base obligatorias
        zona_long  = near_s1 or near_s2 or eq["is_eql"] or near_asia_low  or ob["bull_ob"]
        zona_short = near_r1 or eq["is_eqh"] or near_asia_high or ob["bear_ob"]
        ...
        return {
            ...
            "sl": round(sl, 8), "tp": round(tp, 8), "tp1": round(tp1, 8),
            "atr": round(atr, 8), "score": score, "rsi": rsi,
            "rr":  round(rr, 2), "motivos": motivos, "kz": kz["nombre"],

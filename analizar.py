...
def analizar_par(par: str):
    try:
        ...
        # ──────────────────────────────────────────────────────
        # CONDICIONES BASE  ← FIX#1 y FIX#2
        # OB+FVG es suficiente por sí solo (señal institucional fuerte)
        # Sweep también es base válida (liquidez barrida + reversión)
        # ──────────────────────────────────────────────────────
        ...
        # FIX#1: OB+FVG es base suficiente (no requiere zona adicional)
        # FIX#2: Sweep también activa la base
        base_long  = (fvg["bull_fvg"] and zona_long) or ob_fvg_bull or sweep["sweep_bull"]
        base_short = (fvg["bear_fvg"] and zona_short) or ob_fvg_bear or sweep["sweep_bear"]

        # Filtros de confirmación
        rng_vela      = max(candles[-1]["high"] - candles[-1]["low"], 1e-10)
        # FIX#3: mech_ok incluye también velas con mecha favorable
        ...
        # FIX#5: RSI es suave cuando score es muy alto
        ...
        # FIX#4: log detallado de por qué no hay señal (solo cuando hay score relevante)
        if lado is None:
            if sl_long >= 3 or sl_short >= 3:
                razon_l = []
                razon_s = []
                if not base_long:
                    razon_l.append(f"noBase(fvg={fvg['bull_fvg']},zona={zona_long},sweep={sweep['sweep_bull']},ob_fvg={ob_fvg_bull})")
                if sl_long < score_min:
                    razon_l.append(f"score={sl_long}<{score_min}")
                if not conf_long:
                    razon_l.append("noConf")
                if not mom_long:
                    razon_l.append("noMom")
                if not trend_ok_long:
                    razon_l.append(f"HTF={htf}")
                if not rsi_ok_long:
                    razon_l.append(f"RSI={rsi:.1f}")
                if not base_short:
                    razon_s.append(f"noBase(fvg={fvg['bear_fvg']},zona={zona_short},ob_fvg={ob_fvg_bear})")
                if sl_short < score_min:
                    razon_s.append(f"score={sl_short}<{score_min}")
                if not conf_short:
                    razon_s.append("noConf")

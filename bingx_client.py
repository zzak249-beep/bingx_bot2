...
class BingXClient:
    def __init__(self, api_key: str, api_secret: str, base_url: str, recv_window_ms: int = 5000,
                 timeout: float = 15.0, max_retries: int = 3, demo_mode: bool = False):
        self.api_key = api_key
        self.api_secret = api_secret.encode("utf-8")
        # IMPORTANTE: BingX demo trading (VST) usa una URL BASE DISTINTA
        # (open-api-vst.bingx.com), no un sufijo "-VST" en el símbolo sobre
        # la URL de producción -- confirmado contra varios SDKs oficiales
        # y de terceros. Si demo_mode=True se ignora el base_url pasado
        # (salvo que ya sea el de VST) y se fuerza el correcto, para que
        # nunca sea posible operar contra producción por error de config.
        if demo_mode and "vst" not in base_url.lower():
        ...
        logger.info("BingXClient inicializado contra %s (%s)", self.base_url,
                    "DEMO/VST" if demo_mode else "PRODUCCIÓN REAL")
    ...
    def get_all_tickers(self) -> list[dict]:
        """Estadísticas de 24h de todos los símbolos (para filtrar por
        liquidez antes de vigilar/operar uno en modo SYMBOLS=ALL)."""
    ...
    def get_24h_quote_volumes(self) -> dict[str, float]:
        ...
        try:
            ...
            logger.exception("No se pudieron leer los tickers de 24h para filtrar liquidez")
            return volumes
        for t in tickers:
            sym = t.get("symbol")
            if not sym:
    ...
    def get_funding_rate(self, symbol: str) -> Optional[float]:
        """Funding rate actual/próximo del símbolo. Convención estándar:
        positivo = longs pagan a shorts; negativo = shorts pagan a longs."""
        try:
            data = self._request("GET", "/openApi/swap/v2/quote/premiumIndex", {"symbol": symbol}, signed=False)
            row = data[0] if isinstance(data, list) else data
            return float(row.get("lastFundingRate", row.get("fundingRate", 0)) or 0)
        except Exception:
            logger.warning("No se pudo leer funding rate de %s", symbol)
            return None
    ...
    def has_stop_and_take_profit(self, symbol: str) -> bool:
        """True si hay al menos una orden STOP_MARKET y una
        TAKE_PROFIT_MARKET abiertas para el símbolo -- se llama justo tras
        abrir una entrada para confirmar que BingX aceptó de verdad ambas
        órdenes de protección (puede rechazar una en silencio por deriva
        de precio u otro motivo, dejando la posición desprotegida)."""
        try:
            orders = self.get_open_orders(symbol)
        except Exception:
            logger.exception("No se pudo verificar SL/TP de %s tras abrir la orden", symbol)
            return False

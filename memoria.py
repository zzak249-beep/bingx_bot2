"""
memoria.py — Aprendizaje avanzado v2.1 [FIXED]
Correcciones:
  ✅ Guardado atómico con archivo temporal (evita corrupción)
  ✅ Backup automático antes de cada guardado
  ✅ Recuperación desde backup si el archivo principal está corrupto
  ✅ Soporte para variable de entorno MEMORY_FILE (Railway puede usar /data/)
  ✅ Log de advertencia si el filesystem es efímero
"""
...
# ✅ Usa /data/ si existe (Railway Volumes) o carpeta actual como fallback
_DATA_DIR   = os.getenv("MEMORY_DIR", "/data" if os.path.isdir("/data") else ".")
MEMORY_FILE = os.path.join(_DATA_DIR, "memoria.json")
...
def _cargar():
    ...
    for filepath in (MEMORY_FILE, BACKUP_FILE):
        ...
        try:
            ...
            for k in _data:
                if k in loaded:
                    _data[k] = loaded[k]
            source = "principal" if filepath == MEMORY_FILE else "BACKUP"
            log.info(
                f"[MEMORIA] Cargada ({source}): {len(_data['pares'])} pares | "
            ...
            continue
    log.info("[MEMORIA] Sin datos previos — iniciando desde cero")

def _guardar():
    """✅ Guardado atómico: escribe en .tmp y luego renombra."""
    try:
        ...
        # Intentar guardado directo si falla el atómico
        try:
            with open(MEMORY_FILE, "w") as f:
                json.dump(_data, f, indent=2)
        except Exception as e2:
            log.error(f"[MEMORIA] Error guardado directo: {e2}")
...
# Advertencia si el directorio no es persistente
if _DATA_DIR == ".":
    log.warning(
        "[MEMORIA] ⚠️  Guardando en directorio local — "
        "los datos se perderán en cada redeploy de Railway. "
        "Añade un Volume en Railway montado en /data para persistencia."
    )

"""
config.py — Bellsz Bot v2.0 [Liquidez Lateral] — DEFINITIVO
Todas las variables que necesitan main_bellsz.py y analizar_bellsz.py
"""
import os
...
# ─── Liquidez (núcleo Bellsz) ────────────────────────────────
# LIQ_MARGEN replica margen_pip del Pine Script (se divide /100 internamente)
LIQ_LOOKBACK = int(os.getenv("LIQ_LOOKBACK",  "30"))
LIQ_MARGEN   = float(os.getenv("LIQ_MARGEN",  "0.15"))   # 0.15 = 0.15%
...
# ─── Filtros de entrada ──────────────────────────────────────
SCORE_MIN       = int(os.getenv("SCORE_MIN",      "5"))
...
# ─── Kill Zones (minutos UTC desde medianoche) ───────────────

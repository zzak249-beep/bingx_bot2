...
=========================================================================
FIX DEFINITIVO ERROR 100001:
  - _sign(): query string SIN sorted() — orden natural de inserción
  - _post(): parámetros en QUERY STRING de la URL, body vacío (b"")
    → BingX Swap v2 requiere los params en la URL del POST, no en body JSON
...
# ══════════════════════════════════════════════════════════════════
# EXCHANGE — ONE-WAY MODE  |  FIX FIRMA 100001
# ══════════════════════════════════════════════════════════════════
class BingXClient:
    def __init__(self):
        self.client = httpx.AsyncClient(base_url=BASE_URL, timeout=20)

    # ── FIX: sin sorted(), timestamp al final, signature fuera del hash ──
    def _sign(self, params: dict) -> tuple[str, str]:
        """
        Devuelve (query_string, signature).
        El query string usa el orden de inserción del dict (Python 3.7+).
        'signature' NO se incluye en la cadena firmada.
        """
    ...
    def _add_auth(self, params: dict) -> dict:
        """Añade timestamp y signature al dict de parámetros."""
        p = dict(params)
    ...
    # BingX Perpetual Swap v2: los parámetros deben ir en la URL (query string).
    # Enviarlos en el body JSON produce error 100001 (firma no coincide).
    async def _post(self, path: str, params: dict) -> dict:
        p = self._add_auth(params)
        r = await self.client.post(
            path,
            params=p,       # ← query string
            content=b"",    # ← body vacío
...
def analyze(ticker, candles, candles_1h, funding) -> Optional[CoinResult]:
    ...
    i  = -1
    cn = float(closes[i]); hi = float(highs[i]); lo = float(lows[i])
...
async def run_diagnostics():
    ...
    lines.append(f"*Fix 100001: sin sorted() + POST query string*")
...
async def manage_positions():
    try:
        ...
        for pos in positions:
            ...
            # Cierre parcial 50% al llegar al 1R
...
async def main():
    ...
    await tg(
        f"🔄 *V45 ONE-WAY arrancando...*\n"
        f"TF:`{TIMEFRAME}` Lev:`{LEVERAGE}x`\n"
        f"_Fix 100001: firma sin sorted() + POST query string_"
    )

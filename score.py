...
def compute(
    ...
    cascade: dict | None,
    bias30m: str | None,
    wyckoff_result: "wyckoff.WyckoffConfirm | None" = None,
) -> EntryScore:
    ...
    # Estructura Wyckoff (spring/upthrust + climax de volumen) en la
    # misma vela — igual de opcional que el resto: confirma (+10),
    # sin dato o no confirma (0). No resta si contradice: a diferencia
    # del RSI (que es el mismo indicador que dispara la entrada, así
    # que contradecirlo es una señal fuerte de que el patrón es débil),
    # Wyckoff es una lectura independiente y todavía sin validar con
    # datos propios (ver wyckoff.py) — penalizar con algo no medido
    # sería inventar precisión que no existe.
    if wyckoff_result is not None and wyckoff_result.señal is not None:
        detalle["wyckoff"] = 10.0 if wyckoff.confirms(sig.side, wyckoff_result) else 0.0
    else:
        detalle["wyckoff"] = 0.0

"""
exchange.py — BingX Perpetual Futures REST API v2.0
Usa cantidad fija en USDT por trade (no % del balance)
"""
...
def set_leverage(symbol: str, leverage: int) -> bool:
    if config.MODO_DEMO:
        return True
    try:
        _post("/openApi/swap/v2/trade/leverage", {"symbol": symbol, "side": "LONG",  "leverage": leverage})
        _post("/openApi/swap/v2/trade/leverage", {"symbol": symbol, "side": "SHORT", "leverage": leverage})
        return True
    except Exception as e:
...
def calcular_cantidad(symbol: str, trade_usdt: float, precio: float) -> float:
    ...
    Siempre usa trade_usdt fijo (viene de memoria.get_trade_amount())
    """
...
def abrir_long(symbol: str, qty: float, precio: float, sl: float, tp: float) -> dict:
    ...
    params = {
        ...
        "stopLoss": str(round(sl, 8)), "takeProfit": str(round(tp, 8)),
    }
